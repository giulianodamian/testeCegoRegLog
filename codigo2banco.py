...
# Configurações
PVALUE_THRESHOLD = 0.25  # P-value threshold for initial feature selection
INFLUENCE_THRESHOLD = 0.2  # Threshold for change in coefficient influence
MAX_ITERATIONS = 100000  # Maximum iterations for feature selection
INPUT_FILE = 'bank.csv'  # Changed to 'bank.csv'
OUTPUT_TXT = 'resultados_completos.txt'  # Output text file
OUTPUT_PNG_PREFIX = 'scatter_plot_'  # Prefix for scatter plot filenames
NONLINEAR_THRESHOLD = 0.1  # Threshold for considering non-linearity
HOSMER_LEMESHOW_GROUPS = 10  # Groups for Hosmer-Lemeshow test
COLIN_THRESHOLD = 0.6  # Collinearity threshold
VIF_THRESHOLD = 5  # VIF threshold for multicollinearity
CORR_PLOT = 'matriz_correlacao.png'  # Filename for correlation plot
...
def save_to_txt(content, file_path, mode='a'):
    ...
    Args:
        ...
        mode (str, optional): The file writing mode ('a' for append, 'w' for write). Defaults to 'a'.
    """
    with open(file_path, mode, encoding='utf-8') as f:
        f.write(content + "\n")

def load_and_process():
    """Carrega, baixa (se necessário), e prepara os dados.

    Returns:
        tuple: Um tuple contendo:
            - X (pd.DataFrame): As variáveis preditoras.
            - y (pd.Series): A variável alvo.
            - feature_names (list): A lista de nomes das variáveis preditoras.
    """
    try:
        # Inicializa o arquivo de saída TXT
        ...
        response.raise_for_status()  # Lança uma exceção para erros HTTP (status code != 200)

        # Salva o conteúdo em um novo arquivo CSV localmente
        with open(novo_nome, 'wb') as f:
            f.write(response.content)

        # 1) Importação e processamento dos dados
        bank = pd.read_csv(novo_nome, header=0)
        ...
        bank['education'] = bank['education'].map(education_map).fillna('Unknown')

        # 5) Criando variáveis dummy
        ...
        # Criando X e Y e escalonando
        Y = bank_final['y']
        X = bank_final.drop(columns=['y'])
        colunas = X.columns
        scaler = preprocessing.StandardScaler().fit(X)
        X_scaled = pd.DataFrame(scaler.transform(X), columns=colunas)

        # Garantindo dados numéricos e lidando com NaN
        for col in X_scaled.columns:
            X_scaled.loc[:, col] = pd.to_numeric(X_scaled[col], errors='coerce')
...
def logistic_regression_pvalues(X, y):
    ...
    try:
        ...
        if not model.mle_retvals['converged']:
            print("Aviso: Modelo não convergiu!")
...
def run_univariable_analysis(X, y, feature_names, output_txt):
    ...
    for feature in feature_names:
        ...
        model = sm.Logit(y, X_uni).fit(disp=0, maxiter=1000)
...
def multivariable_model_comparison(X, y, initial_features, output_txt):
    ...
    model_full = sm.Logit(y, X_multi).fit(disp=0, maxiter=1000)
    ...
    while changed and iteration < MAX_ITERATIONS:
        ...
        if max_p_value > 0.05:  # Usando 0.05 como limiar de significância
            ...
            model_reduced = sm.Logit(y, X_reduced).fit(disp=0, maxiter=1000)
            ...
            if lr_pvalue > 0.05:  # Não há diferença significativa
                ...
                if any(change > 0.2 for change in coef_change.values()):  # Se alguma mudança > 20%
                    ...
                    model_full = sm.Logit(y, sm.add_constant(X[current_features])).fit(disp=0, maxiter=1000)
...
def calculate_deviance(y_true, y_pred):
    ...
    y_pred = np.clip(y_pred, 1e-10, 1 - 1e-10)  # Evita valores extremos para log
...
def calculate_pearson_chi2(y_true, y_pred):
    """Calcula a estatística de Pearson Chi-square.
       ...
       Args:
        y_true (pd.Series): Valores observados da variável alvo.
        y_prob (np.array): Probabilidades previstas pelo modelo.
    Returns:
        float: O valor da estatística de Pearson Chi-square.
    """
...
def plot_correlation_matrix(X, threshold, output_txt, corr_plot_name):
    ...
    Args:
        X (pd.DataFrame): As variáveis preditoras.
        threshold (float): O limiar de correlação para destacar.
        output_txt (str): Caminho para salvar o relatório em TXT.
        corr_plot_name(str): Nome do arquivo para salvar a matriz de correlação
    """
...
def remove_highly_correlated(X, threshold, output_txt):
    ...
    Args:
        X (pd.DataFrame): As variáveis preditoras.
        threshold (float): O limiar de correlação.
        output_txt (str): Caminho para salvar o relatório em TXT.

    Returns:
        tuple: Um tuple contendo:
            - pd.DataFrame: As variáveis preditoras após a remoção de colinearidade.
            - pd.DataFrame: Um relatório das variáveis removidas.
    """
    save_to_txt(f"\nTRATAMENTO DE COLINEARIDADE (≥{threshold * 100}%)", output_txt)

    corr_matrix = X.corr().abs()  # Calcula a matriz de correlação absoluta
    upper = corr_matrix.where(np.triu(np.ones(corr_matrix.shape), k=1).astype(bool))  # Máscara o triângulo superior
    to_drop = [col for col in upper.columns if any(upper[col] > threshold)]  # Identifica colunas a remover
    ...
    X_filtered = X.drop(columns=to_drop, errors='ignore')  # Remove as colunas identificadas
...
def remove_high_vif(X, threshold, output_txt):
    ...
    Args:
        X (pd.DataFrame): As variáveis preditoras.
        threshold (float): O limiar de VIF.
        output_txt (str): Caminho para salvar o relatório em TXT.

    Returns:
        tuple: Um tuple contendo:
            - pd.DataFrame: As variáveis preditoras após a remoção de multicolinearidade.
            - pd.DataFrame: Um relatório das variáveis removidas.
    """
...
def run_final_model(X, y, output_txt):
    ...
    Args:
        X (pd.DataFrame): As variáveis preditoras (após seleção e tratamento).
        y (pd.Series): A variável alvo.
        output_txt (str): Caminho para salvar o relatório em TXT.

    Returns:
        tuple: Um tuple contendo:
            - statsmodels.discrete.discrete_model.BinaryResults: O resultado do modelo logístico ajustado.
            - sklearn.feature_selection.RFECV: O objeto RFECV após a seleção de features.
            - pd.DataFrame: Um DataFrame com o ranking das features.
    """
    ...
    model = linear_model.LogisticRegression(max_iter=10000, class_weight='balanced')
    rfecv = RFECV(
        estimator=model,
        step=1,
        cv=StratifiedKFold(5),
        scoring='accuracy',
        min_features_to_select=12  # Seleciona 12 features
    )
...
def main():
    ...
    try:
        ...
        # 8. Verifica não-linearidade
        df = X.copy()
        df['target'] = y
        nonlinear_features = plot_scatter_smooth(df, feature_names, OUTPUT_TXT)
        if nonlinear_features:
            print(f"\nFeatures não-lineares detectadas: {list(nonlinear_features.keys())}")

        # 9. Modelagem Final com RFECV
        final_result, rfecv, ranking_df = run_final_model(X_vif, y, OUTPUT_TXT)

        # 10. Salva resultados adicionais
        ...
        print(f"Resultados salvos em: {OUTPUT_TXT} e ranking_features.csv, features_selecionadas.csv")
